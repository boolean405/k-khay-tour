...
const addPermit = async (req, res, next) => {
  ...
  if (dbRole) {
    ...
    if (dbExistPermit) {
      ...
      Helper.fMsg(
        res,
        `${dbPermit.name} Permission added to ${dbRole.name} Role`,
        result,
      );
    }
...
const removePermit = async (req, res, next) => {
  ...
  if (dbRole) {
    ...
    if (dbExistPermit) {
      ...
      Helper.fMsg(
        res,
        `${dbPermit.name} Permission removed from ${dbRole.name} Role`,
        result,
      );
    } else {
      next(
        new Error(
          `Not found input Permission to delete from ${dbRole.name} Role`,
        ),
      );
    }
  } else if (!dbRole && dbPermit) {
