...
const add = async (req, res, next) => {
  ...
  if (dbChildCategory) {
    ...
    if (subCat) {
      ...
      await SubCategoryDB.findByIdAndUpdate(subCat._id, {
...
const drop = async (req, res, next) => {
  let dbChildCategory = await DB.findById(req.params.id);
  if (dbChildCategory) {
    await SubCategoryDB.findByIdAndUpdate(dbChildCategory.sub_category._id, {
      $pull: { child_categories: dbChildCategory._id },
    });
