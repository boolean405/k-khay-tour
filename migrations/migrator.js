  ...
  if (data) {
    ...
    if (defaultData.sub_categories) {
      defaultData.sub_categories.forEach(async (sub_category) => {
        let dbSubCategoryName = await SubCategoryDB.findOne({
    ...
    if (defaultData.child_categories) {
      defaultData.child_categories.forEach(async (child_category) => {
        let dbChildCategoryName = await ChildCategoryDB.findOne({
    ...
    if (dbUser.roles.length > 0) {
      dbUser.roles.forEach(async (role) => {
        if (role.equals(dbRole._id)) {
          console.log(
            `Skipped, ${dbRole.name} Role is already exist in ${dbUser.name} User`,
          );
        } else {
          await UserDB.findByIdAndUpdate(dbUser._id, {
            $addToSet: { roles: dbRole._id },
          });
...
const addCategoryToProduct = async (cat, product) => {
  ...
  if (dbCat && dbProduct) {
    if (dbCat._id.equals(dbProduct.category)) {
      console.log(
        `Skipped, ${dbCat.name} Category is already exist in ${dbProduct.name} Product Category`,
      );
    } else {
      await ProductDB.findByIdAndUpdate(dbProduct._id, {
        $push: { category: dbCat._id },
      });
      console.log(
        `Success, ${dbCat.name} Category is added to ${dbProduct.name} Product Category`,
      );
    }
  }
};
...
const addSubCatToProduct = async (sub_cat, product) => {
  ...
  if (dbSubCat && dbProduct) {
    if (dbSubCat._id.equals(dbProduct.sub_category)) {
      console.log(
        `Skipped, ${dbSubCat.name} Sub Category is already exist in ${dbProduct.name} Product Sub Category`,
      );
    } else {
      await ProductDB.findByIdAndUpdate(dbProduct._id, {
        $push: { sub_category: dbSubCat._id },
      });
      console.log(
        `Success, ${dbSubCat.name} Sub Category is added to ${dbProduct.name} Product Sub Category`,
      );
    }
  }
};
    ...
    if (dbProduct.tags.length > 0) {
      dbProduct.tags.forEach(async (ltag) => {
        if (ltag.equals(dbTag._id)) {
          console.log(
            `Skipped, ${dbTag.name} Tag is already exist in ${dbProduct.name} Product`,
          );
        } else {
          await ProductDB.findByIdAndUpdate(dbProduct._id, {
            $push: { tags: dbTag._id },
          ...
          console.log(
            `Success, ${dbTag.name} Tag is added to ${dbProduct.name} Product`,
          );
        }
      });
    } else {
...
const addSubCatToChildCat = async (sub_cat, child_cat) => {
  ...
  if (dbSubCat && dbChildCat) {
    if (dbSubCat._id.equals(dbChildCat.sub_category)) {
      console.log(
        `Skipped, ${dbSubCat.name} Sub Category is already exist in ${dbChildCat.name} Child Category`,
      );
    } else {
      await ChildCategoryDB.findByIdAndUpdate(dbChildCat._id, {
        $push: { sub_category: dbSubCat._id },
      });
      console.log(
        `Success, ${dbSubCat.name} Sub Category is added to ${dbChildCat.name} Child Category`,
      );
    }
  }
};
    ...
    if (dbSubCat.child_categories.length > 0) {
      dbSubCat.child_categories.forEach(async (child_category) => {
        if (child_category.equals(dbChildCat._id)) {
          console.log(
            `Skipped, ${dbChildCat.name} Child Category is already exist in ${dbSubCat.name} Sub Category`,
          );
        } else {
          await SubCategoryDB.findByIdAndUpdate(dbSubCat._id, {
            $push: { child_categories: dbChildCat._id },
          });
          console.log(
            `Success, ${dbChildCat.name} Child Category is added to ${dbSubCat.name} Sub Category`,
          );
        }
      });
    } else {
